...
int reliable_read(int fd, void* buf, size_t size)
{
  ...
  {
    starttime = MPI_Wtime();
    int rc = SYSIO_INTERFACE_NAME(read)(fd, (char*) buf + n, size - n);
    endtime = MPI_Wtime();

    if (rc > 6)
        printf("%d times: rank %d read %d bytes %0.7f seconds\n", t, rank, rc, endtime - starttime);
...
int reliable_write(int fd, const void* buf, size_t size)
{
  ...
  {
    starttime = MPI_Wtime();
    int rc = SYSIO_INTERFACE_NAME(write)(fd, (char*) buf + n, size - n);
    endtime = MPI_Wtime();
    if (rc  > 6)
        printf("%d times: rank %d wrote %d bytes %0.7f seconds\n", t, rank, rc, endtime - starttime);
...
void checkdata(char* file, size_t size, int times)
{
  ...
  if (times > 0) {
   ...
   for(i=0; i < times; i++) {
      ...
      int fd_me = SYSIO_INTERFACE_NAME(open)(file, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
      ...
      if (fd_me >= 0) {
        ...
        rc = SYSIO_INTERFACE_NAME(fsync)(fd_me);
        if (rc < 0) {
          valid = 0;
        }

        /* make sure the close is without error */
        rc = SYSIO_INTERFACE_NAME(close)(fd_me);
        if (rc < 0) {
          valid = 0;
        }
      }
      ...
      fd_me = SYSIO_INTERFACE_NAME(open)(file, O_RDONLY);
      if (fd_me >= 0) {
        valid = 1;
      ...
      if (read_rank != rank || read_timestep != i) {
        printf("read_rank:%d, INVALID HEADER on rank %d in step %d\n", read_rank, rank, i);  fflush(stdout);
        MPI_Abort(MPI_COMM_WORLD, 0);
      }
 	...
 	SYSIO_INTERFACE_NAME(unlink)(file);
    }
...
int main (int argc, char* argv[])
{
 ...
 err = SYSIO_INTERFACE_NAME(mount)("/data/yzhu/bbfs", "/cruise/", "cruise", 2, size);
 if (err) {
                printf("mount cruise failed\n");
                return err;
 }
 printf ("mount successfully\n");
  ...
  char name[256];
   sprintf(name, "cruise:/cruise/rank.%d", rank);
  ...
  SYSIO_INTERFACE_NAME(umount)("cruise:/cruise");
