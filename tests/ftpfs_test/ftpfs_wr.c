...
#include "sysio.h"
/*
 * Write/read data to/from by ftp(input size KB)
 *
 * Usage: ./ftpfs_wr <size>
 *
 * Cautious: be sure you have the 
 * read and write permission on the link you provided.
 * The link used in the this file, is only an example.
 */
#include <time.h>
...
{
	int	rc, err, size, fd;
	const char *fpath = "ftpfs:/ftpfs/test.txt";
	...
	err = SYSIO_INTERFACE_NAME(mount)("ftp://ftp.sunet.se/", "/ftpfs/", "ftpfs", 2, NULL);
	if (err) {
	...
	if (rc < 0) {
		fprintf(stderr, "ftpfs write failed\n");
		return 0;
	}
	
	err = SYSIO_INTERFACE_NAME(fsync)(fd);
	if (err < 0) {
		fprintf(stderr, "ftpfs fsync failed\n");
		return 0;
	}
	...
	if (err < 0) {
		fprintf(stderr, "ftpfs close failed\n");
		return 0;
	}
	...
	if (err < 0) {
		fprintf(stderr, "ftpfs read failed\n");
		return 0;
	}
	...
	gettimeofday(&end, NULL);	
	time = end.tv_sec + (end.tv_usec/1000000.0) - start.tv_sec - (start.tv_usec/1000000.0);
	printf("bbfs %d read %d bytes %lf seconds\n", err, time);
	...
	if (err < 0) {
		fprintf(stderr, "ftpfs unlink failed\n");
		return 0;
	}
	

	err = SYSIO_INTERFACE_NAME(umount)("ftpfs:/ftpfs");
	if (read_data)
		free(read_data);
	if (data)
		free(data);
