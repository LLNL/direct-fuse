...
struct inode_ops {
    ...
    int (*inop_close)(struct pnode *ino);
...
//    int (*inop_read)(struct pnode *pno, struct ioctx *ioctx);
    int (*inop_read)(void *buf, size_t nbytes, _SYSIO_OFF_T off, struct pnode *pno);

    int (*inop_write)(void *buf, size_t nbytes, _SYSIO_OFF_T off, struct pnode *pno);
//    int (*inop_write)(struct pnode *pno, struct ioctx *ioctx);
    ...
    int (*inop_sync)(struct pnode *pno);
    int (*inop_datasync)(struct pnode *pno);
...
#endif
    int (*inop_ftruncate)(struct pnode *pno, size_t size);
