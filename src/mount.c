...
/*
 * Find mounted filesystem by prefix
 */
int 
_sysio_find_mount(char *path, struct mount **mntp)
{
	struct mount *mnt;
	struct filesys *fs;
	...
	while (mnt) {
		fs = mnt->mnt_fs;
		if (strncmp(path, fs->fs_name, fs->name_len) == 0) {
			*mntp = mnt;
			return 0;
		}
		mnt = mnt->mnt_link.le_next;
	}
		...
		struct mount **mntp)
{
	...
	if (tocover->p_cover) {
		char *path = _sysio_pb_path(tocover->p_cover->p_base, '/');
		path = _sysio_pb_path(mnt->mnt_root->p_base, '/');
	}
		  ...
		  const void *data)
{
	...
	err = (*fssw->fssw_ops.fsswop_mount)(source, flags, data, fstype, NULL, &mnt);
	if (err)
		return err;
	     ...
	     const void *data)
{
	...
	err = _sysio_namei(cwd, target, mountflags, &intent, &tgt);
	if (err)
		return err;
...
SYSIO_INTERFACE_NAME(umount)(const char *target)
{
	...
	err = _sysio_find_mount(target, &mnt);
	if (err)
		goto out;

	target = target + mnt->mnt_fs->name_len + 1;

	/*
	 * Look up the target path node.
	 */
	err = _sysio_namei(mnt->mnt_root, target, 0, NULL, &pno);
