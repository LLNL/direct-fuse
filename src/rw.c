...
SYSIO_INTERFACE_NAME(read)(int fd, void *buf, size_t count)
{
	...
	err = (*ino->i_ops.inop_read)(buf, count, fil->f_pos, fil->f_pno);
	if (err >= 0)
		fil->f_pos = fil->f_pos + err;
				        ...
				        _SYSIO_OFF_T offset)
{
        ...
        err = (*ino->i_ops.inop_read)(buf, count, offset, fil->f_pno);
...
void time_diff(struct timespec end, struct timespec start )
{
	double time;
	 time = end.tv_sec + end.tv_nsec - start.tv_sec - start.tv_nsec;
        printf("%d: %lf seconds\n", pr_count, time);
	pr_count ++;
...
SYSIO_INTERFACE_NAME(write)(int fd, const void *buf, size_t count)
{
	...
	err = (*ino->i_ops.inop_write)(buf, count, fil->f_pos, fil->f_pno);
	if (err >= 0)
		fil->f_pos = fil->f_pos + err;
	return err;
					 ...
					 _SYSIO_OFF_T offset)
{
        ...
        err = (*ino->i_ops.inop_write)(buf, count, offset, fil->f_pno);
