...
do_truncate(struct pnode *pno, struct inode *ino, _SYSIO_OFF_T length)
{
  ...
  err = (*ino->i_ops.inop_ftruncate)(pno, length);
  if (err < 0)
    return err;
					   ...
					   _SYSIO_OFF_T length)
{
  ...
  /*err = _sysio_find_mount(fpath, &mnt);
  if (err)
    goto out;
  fpath = fpath + mnt->mnt_fs->name_len + 1;

  INTENT_INIT(&intent, INT_UPDPARENT, NULL, NULL);
  err = _sysio_namei(mnt->mnt_root, fpath, ND_NEGOK, &intent, &pno);
  if (err)
    goto out;
...
PREPEND(_, SYSIO_INTERFACE_NAME(ftruncate))(int fd, _SYSIO_OFF_T length)
{
	...
	err = do_truncate(fil->f_pno, fil->f_ino, length);
out:
	SYSIO_INTERFACE_RETURN(err ? -1 : 0, err);
}
