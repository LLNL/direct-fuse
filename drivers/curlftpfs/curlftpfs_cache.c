...
static int cache_getattr(const char *path, struct stat *stbuf)
{
    ...
    if (err == -EAGAIN) {
        ...
        if (!err)
            ftpfs_cache_add_attr(path, stbuf);
        else if (err == -ENOENT)
            ftpfs_cache_add_attr(path, NULL);
    }
    return err;
}
                         ...
                         const struct stat *stbuf)
{
    ...
    if (!err) {
        ...
        ftpfs_cache_add_attr(fullpath, stbuf);
        g_free(fullpath);
    }
    return err;
}
...
void ftpfs_cache_unity_fill(struct fuse_cache_operations *oper,
...
struct fuse_operations *ftpfs_cache_init(struct fuse_cache_operations *oper)
{
    ...
    ftpfs_cache_unity_fill(oper, &cache_oper);
    if (cache.on) {
