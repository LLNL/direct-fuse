...
static int ftpfs_open(const char* path, struct fuse_file_info* fi) {
  int fd = ftpfs_open_common(path, 0, fi);
 return fd;
}
...
{
#if FUSE_VERSION >= 26
    return fuse_main(args->argc, args->argv, ftpfs_cache_init(&ftpfs_oper), NULL);
#else
    return fuse_main(args->argc, args->argv, ftpfs_cache_init(&ftpfs_oper));
#endif
}
...
int ftpfs_sys_init(int argc, char** argv) {
  int res;
